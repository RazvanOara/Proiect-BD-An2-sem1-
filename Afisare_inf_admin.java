...
public class Afisare_inf_admin {

	public Afisare_inf_admin() {
	...
	static JFrame f;
	static JTextField textnume, textprenume, textadresa, textcnp, parola, username;
	static JButton editi;
	static JLabel nu, pn, ad, cnp, y, pa, us;
	static String minHour, maxHour;
	static boolean edit;

	public void afisare_date(int idadmin, String userName, String userPassword, int userid, JFrame mare, JFrame logIn,
			int isSuper) {
				...
				if (edit) {
					...
					if (ok == JOptionPane.YES_OPTION) {
						try {
							...
							rs2 = selectStatement.executeQuery("call u.update_data_admin(" + idadmin + ",'"
									+ textnume.getText() + "','" + textprenume.getText() + "','" + textcnp.getText()
									+ "','" + textadresa.getText() + "'," + isSuper + ");");
						
							rs3 = selectStatement2.executeQuery("call u.update_data_user (" + userid + ",'"
									+ username.getText() + "','" + parola.getText() + "','admin')");
